  ...
  async trigger(this: ITriggerFunctions): Promise<ITriggerResponse> {
      ...
      try {
        ...
        webhookData.unsubscribeFn = () => {
          ...
          cleanupFirebaseApp(credentials.projectId as string)
            .catch((e: Error) => console.error('Error cleaning up Firebase app:', e));
        };
        ...
        if (operation === 'listenToCollection') {
          ...
          try {
            ...
            unsubscribeFn = query.onSnapshot(
              (snapshot: any) => {
                console.log(`Received snapshot with ${snapshot.docChanges ? snapshot.docChanges().length : 0} changes`);
                ...
                snapshot.docChanges().forEach((change: any) => {
                  ...
                  try {
                    ...
                    const metadata = {
                      hasPendingWrites: 
                        (change.doc.metadata && typeof change.doc.metadata.hasPendingWrites !== 'undefined') 
                    ...
                    console.log(`Document data:`, JSON.stringify(docData).substring(0, 200) + (JSON.stringify(docData).length > 200 ? '...' : ''));
    ...
    return {
      ...
      manualTriggerFunction: (async () => {
        ...
        return [[{ json: { success: true, message: 'Firestore trigger is active and listening for changes.' } }]];
      }) as any
    };
  }
}
